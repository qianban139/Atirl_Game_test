...
| 筛选项 | 选择 |
|--------|------|
| GPU 型号 | **RTX 3080**（性价比之王，¥30 预算首选） |
| 计费方式 | **可中断实例（Spot）**（价格是按量的 30-50%，¥30 能跑完 50M） |
| 地区 | 选离你近的（北京/内蒙/广州 任选） |

> **预算 ¥30 推荐 3080 Spot，¥0.4-0.5/时，¥30 = 60+ 小时，轻松跑完 50M。**
> 详见 `reference/budget-gpu-comparison.md`。
...
| 数据盘 | **50GB** | 默认 50GB 足够（checkpoint ~15MB/个） |
| 计费方式 | **可中断实例** | Spot 价格是按量的 30-50%，¥30 预算的关键 |
...
| 项目 | 计算 | 金额 |
|------|------|------|
| 3080 Spot 训练 50M | ~35h × ¥0.45 | **≈ ¥16** |
| 消融实验 50M | ~35h × ¥0.45 | ≈ ¥16 |
| 数据盘 50GB/天 | ¥0.0035/GB/天 × 50GB | ≈ ¥0.18/天 |
| **合计（串行）** | ~70h = ~3 天 | **≈ ¥32** |

> **¥30 够你跑完 PPO+ICM 50M，还能剩一半预算跑消融。**实际时间取决于 3080 FPS，预留 +20% buffer。

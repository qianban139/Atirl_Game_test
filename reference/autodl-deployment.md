...
| 配置项 | 推荐 | 说明 |
|--------|------|------|
| GPU | **RTX 3080 (10GB)** | ¥30 预算最佳性价比 |
| 计费 | **Spot 可中断实例** | ¥0.4-0.5/时，按量的 30-50% |
| CPU | ≥ 12 核 | 8 并行 env 推理 |
| 内存 | ≥ 32GB | Buffer + 8 个 Atari 实例 |
| 系统盘 | ≥ 30GB | 系统 + 依赖 + ROM |
| 数据盘 | 50GB | checkpoint + 日志绰绰有余 |

> **¥30 预算能跑完**：3080 Spot ~¥0.45/h × 70h = ¥31.5，足够 50M 主实验 + 消融实验。
> 详见 `reference/budget-gpu-comparison.md`。

**省钱技巧**：务必选**可中断实例**，不要选按量。每 500 episode 有 checkpoint，被回收也不怕。

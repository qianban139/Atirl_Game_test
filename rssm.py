...
from networks import symlog, twohot_encode


def sample_categorical(logits, unimix=0.01):
    """Straight-through categorical sampling with unimix."""
...
class RSSM(nn.Module):
    ...
    def forward_step(self, h, z, a, x=None):
        """Single RSSM step. If x: uses posterior. Otherwise: prior (imagination)."""
        z_flat = z.reshape(z.shape[0], -1)
        a_onehot = F.one_hot(a, self.cfg.num_actions).float()
        h_next = self.gru(torch.cat([z_flat, a_onehot], dim=-1), h)
        logits = self.posterior(h_next, self.encoder(x)) if x is not None else self.prior(h_next)
    ...
    def observe_sequence(self, obs, actions, rewards, dones):
        ...
        recon_list, reward_logits_list, cont_logits_list = [], [], []
        ...
        for t in range(T):
            ...
            cont_logit = self.continue_head(features)
            ...
            cont_logits_list.append(cont_logit)
        ...
        return {
            ...
            "reward_logits": torch.stack(reward_logits_list, 1),
            "cont_logits": torch.stack(cont_logits_list, 1),
        }
    ...
    def compute_world_model_loss(self, obs, actions, rewards, dones):
        ...
        # Reconstruction: MSE in symlog space (decoder outputs raw symlog)
        obs_symlog = symlog(obs)
        L_recon = F.mse_loss(out["recon"], obs_symlog, reduction='mean')

        # Reward: TwoHot cross-entropy
        rew_symlog = symlog(rewards)
        rew_target = twohot_encode(rew_symlog, self.cfg.critic_bins, self.cfg.critic_low, self.cfg.critic_high)
        L_reward = -(rew_target * F.log_softmax(out["reward_logits"], dim=-1)).sum(-1).mean()

        # Continue: BCE with logits (target = 1 - done)
        cont_target = (1 - dones.float())
        L_cont = F.binary_cross_entropy_with_logits(out["cont_logits"], cont_target)

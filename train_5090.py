...
def train():
    ...
    print(f"[Setup] 5090 Profile: {config.num_envs} envs × {config.rollout_steps} steps = {config.batch_size} transitions/batch")
    ...
    param_count += sum(p.numel() for p in inverse_model.parameters())
    param_count += sum(p.numel() for p in forward_model.parameters())
    print(f"[Setup] Total parameters: {param_count:,}")
